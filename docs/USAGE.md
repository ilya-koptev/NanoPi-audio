...
|----------------|----------|-------------|-------|
| `audio/track`  | имя      | вход | играть файл по имени **без расширения** (`sovy` → sovy.mp3, `2` → 2.wav); если нет — ошибка в `audio/log` |
...
```bash
mosquitto_pub -h <ip> -t audio/track  -m sovy  # играть sovy.* (или -m 2 для 2.*)
...
В `audio/track` указывайте имя файла **без расширения**: `sovy` сыграет `sovy.mp3`,
`2` сыграет `2.wav`. Если файла нет — в `audio/log` пишется ошибка.
...
Внизу веб-страницы — строка версии: «Установлена последняя версия» либо «Доступно
обновление: X → Y» с кнопкой **«Обновить»**. Проверка идёт по последнему релизу на
GitHub (без GitHub Pages и apt-репозитория).

Обновить можно:
- кнопкой **«Обновить»** в веб-морде, либо
- вручную: `sudo /usr/local/bin/nanopi-audio-update.sh` — скачивает `.deb` последнего
  релиза с GitHub и ставит его.
